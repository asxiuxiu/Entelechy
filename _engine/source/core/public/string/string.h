...
public:
    ...
    char operator[](usize i) const { return m_str[i]; }
    usize length() const { return m_str.size(); }
    usize size() const { return m_str.size(); }
    bool empty() const { return m_str.empty(); }
    bool isInline() const { return m_str.size() <= SSO_CAPACITY; }
    ...
    BasicString& operator+=(const char* str) { return append(str); }
    BasicString& operator+=(const BasicString& other) { return append(other); }
    BasicString& operator+=(StringView sv) { return append(sv); }
    BasicString& operator+=(char ch) { return append(ch); }
    ...
    usize find(const char* str, usize pos = 0) const {
        ...
        if (subLen > m_str.size() || pos >= m_str.size()) return npos;
    ...
    usize find(StringView sv, usize pos = 0) const {
        ...
        if (sv.length() > m_str.size() || pos >= m_str.size()) return npos;
    ...
    BasicString substr(usize pos = 0, usize len = npos) const {
        if (pos >= m_str.size()) return BasicString();
    ...
    StringView view(usize pos, usize len = npos) const {
        if (pos >= m_str.size()) return StringView();
        usize actualLen = (len == npos || pos + len > m_str.size()) ? (m_str.size() - pos) : len;
...
template<typename AllocatorT>
inline StringView::StringView(const BasicString<AllocatorT>& s)
