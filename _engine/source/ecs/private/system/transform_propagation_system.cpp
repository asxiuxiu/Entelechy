...
void TransformPropagationSystem::tick(World &world, FrameArena &arena, f32 dt)
{
    ...
    for (auto [e, trans] : Query<Transform>(world))
    {
        ...
        while (current.valid())
        {
            ...
            if (!childOf || !childOf->parent.valid())
                break;
    ...
    for (auto [e, trans, global] : Query<Transform, GlobalTransform>(world))
    {
        if (!trans || !global)
            continue;
        if (!world.hasComponent<TransformTreeChanged>(e))
            continue;
    ...
    for (u32 r = 0; r <= maxRank; ++r)
    {
        ...
        {
            m_thread_pool->parallelFor(bucket.size(), 16, [&](usize i) { propagateOne(bucket[i]); });
        }
        else
        {
            for (Entity e : bucket)
