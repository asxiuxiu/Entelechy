...
FileOutput::FileOutput(const LogFileConfig &config) : m_config(config), m_current_size(0) {}
...
void FileOutput::write(const QueuedLogEntry &entry)
{
    ...
    int lineLen = std::snprintf(lineBuf, sizeof(lineBuf), "%s [%s][%s::%s] %s\n", timeBuf,
                                logLevelToString(entry.m_level), logFileBasename(entry.m_file),
...
void FileOutput::rollFile()
{
    ...
    char oldestPath[256];
    std::snprintf(oldestPath, sizeof(oldestPath), "%s.%u", m_config.m_base_path.c_str(), m_config.m_max_files);
    ...
    for (int i = static_cast<int>(m_config.m_max_files) - 1; i >= 1; --i)
    {
        char oldPath[256];
        char newPath[256];
        std::snprintf(oldPath, sizeof(oldPath), "%s.%d", m_config.m_base_path.c_str(), i);
        std::snprintf(newPath, sizeof(newPath), "%s.%d", m_config.m_base_path.c_str(), i + 1);
