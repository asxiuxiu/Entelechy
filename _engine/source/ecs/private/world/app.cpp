        ...
        for (IPlugin *existing : m_plugins)
        {
            if (std::strcmp(existing->name(), plugin->name()) == 0)
            {
                LOG_WARN(LogCategories::kEngine,
                         "Plugin '%s' is unique and already registered; ignoring duplicate add.", plugin->name());
                plugin->~IPlugin();
                DefaultAllocator::free(plugin);
                return;
            }
    ...
    for (auto &group : phaseGroups)
    {
        ...
        for (usize li = 0; li < gn; ++li)
        {
            ...
            for (const auto &depName : p->dependencies())
            {
                ...
                if (!depGlobalIdx)
                {
                    LOG_WARN(LogCategories::kEngine, "Plugin '%s' declares dependency '%s' which is not registered",
        ...
        {
            LOG_WARN(LogCategories::kEngine, "App: cycle detected in plugin dependency graph for phase %d",
                     group.phaseIndex);
        }
...
void App::build()
{
    ...
    for (IPlugin *plugin : m_plugins)
    {
        LOG_DEBUG(LogCategories::kEngine, "  Plugin build: %s (phase=%d)", plugin->name(),
                  static_cast<int>(plugin->phase()));
        plugin->build(*this);
    }
    ...
    while (!all_ready)
    {
        ...
        {
            LOG_ERROR(LogCategories::kEngine, "App::build() -- plugins stuck in ready() poll, aborting.");
            break;
        }
    }
