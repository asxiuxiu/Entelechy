...
- 向上依赖：
  ...
  - Core（基础类型 `f32`）
- 被依赖：
  - Runtime（主循环调用 DockSpace/Debug/Log 面板）
  - [Editor 模块](../editor/AGENTS.md)（ECS Inspector 基于 ImGui 绘制）

## 架构决策
- 目前开启 `ImGuiConfigFlags_DockingEnable`，关闭 `ViewportsEnable`（面板限制在窗口内）
- DPI 缩放通过 `glfwGetWindowContentScale` 自动适配
- **ImGuiLib 不依赖 EcsLib**：ECS Inspector 面板和 Atom 绘制注册已迁至 [Editor 模块](../editor/AGENTS.md)，ImGuiLib 保持为纯 UI 基础设施层
