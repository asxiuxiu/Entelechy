...
World::World(IAllocator *allocator) : m_allocator(allocator), m_owns_registry(true)
{
...
World::World(EntityRegistry &registry, IAllocator *allocator)
    : m_allocator(allocator), m_registry(&registry), m_owns_registry(false)
{
}
...
void World::setParentImmediate(Entity child, Entity parent)
{
    if (!valid(child))
        return;
    if (parent.valid() && !valid(parent))
        return;
    if (parent.valid() && isDescendant(*this, child, parent))
        return; // cycle guard
...
void printWorld(const World &world)
{
    ...
    for (u32 id = 0; id < world.maxEntityID(); ++id)
    {
        ...
        if (world.valid(e))
        {
            ...
            printf("Entity %u: ", id);
            if (pos)
                printf("Position(%.1f, %.1f) ", pos->x, pos->y);
            if (vel)
                printf("Velocity(%.1f, %.1f) ", vel->vx, vel->vy);
            if (health)
                printf("Health(%.1f) ", health->hp);
            printf("\n");
        }
    }
    printf("\n");
