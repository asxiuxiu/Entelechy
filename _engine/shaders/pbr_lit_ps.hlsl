...
cbuffer PerFrame : register(b0)
{
    float3 uViewPos;
    float  _pad0;
    float3 uLightDir;       // direction the light travels
...
float4 main(PSInput input, bool isFrontFace : SV_IsFrontFace) : SV_Target0
{
    ...
    float3 albedo = pow(uColor * tex.rgb, float3(2.2, 2.2, 2.2));
    ...
    float3 V = normalize(uViewPos - input.vWorldPos);
    ...
    float3 color = uAmbient * albedo + Lo;
