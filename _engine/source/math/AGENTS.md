...
| `transform_component.h` | ECS 组件 `Transform`（本地 TRS）/`GlobalTransform`（世界矩阵缓存）/`TransformTreeChanged`（层级脏标记） |
| `hierarchy.h` | 层级辅助函数：`hierarchyDepth`、`attachChild`、`detachChild`、`isDescendant`、`collectDescendants` |
...
- **ECS 集成**：`Transform` / `GlobalTransform` / `TransformTreeChanged` 分离；`TransformPropagationSystem` 每帧按深度拓扑排序全量重算（小场景策略），所有数学类型保持纯 POD。
- **层级传播**：`ChildOf`/`Children` 双向关系驱动父子矩阵级联（`global = parent_global * local`）。
