...
void FlyCameraSystem::tick(Entelechy::World &world, Entelechy::FrameArena &, f32 dt)
{
    ...
    // -- Consume discrete input events to maintain held/drag state ---------
    for (const RawInputEvent &e : InputQueue::instance().events())
    {
        switch (e.type)
        {
        case RawInputEvent::KeyPress:
        case RawInputEvent::KeyRelease:
            if (e.keyCode >= 0 && e.keyCode < MAX_TRACKED_KEYS)
                m_key_held[e.keyCode] = (e.type == RawInputEvent::KeyPress);
            break;
    ...
    for (auto [entity, tag, transform] : Query<FlyCameraTag, Transform>(world))
    {
        ...
        if (m_key_held[GLFW_KEY_A])
            move -= right;
        if (m_key_held[GLFW_KEY_E])
            move += Vec3{0.0f, 1.0f, 0.0f};
        if (m_key_held[GLFW_KEY_Q])
            move -= Vec3{0.0f, 1.0f, 0.0f};

        f32 speed = MOVE_SPEED;
        if (m_key_held[GLFW_KEY_LEFT_SHIFT] || m_key_held[GLFW_KEY_RIGHT_SHIFT])
            speed *= BOOST_FACTOR;
