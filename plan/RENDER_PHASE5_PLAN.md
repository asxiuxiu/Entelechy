...
> **落地情况（2026-08-06）**：范围 1–3 全部落地。天空（D6）：主 World 新增 `SkySettings`（zenith/horizon/enabled，`render_system/public/components/SkySettings.h`）+ 组件注册 + `ExtractSkySystem`（取第一个，同相机/光照模式）→ Render World `ExtractedSky`（`RenderSky.h`）；`RenderExecuteSystem` 自持天空 Material + 全屏三角形 VBO（3 顶点 NDC，loc 0 vec2），clear 后、opaque 前绘制——vs 以 `uInvViewProj`（`(proj*view).inverse()`）重建远平面世界位置、NDC z=1 钉远平面（depthTest + LessEqual、depthWrite off、cull None），fs 按视线 y 分量 `mix(horizon, zenith, clamp(dir.y,0,1))`、输出与 lit PBR 相同的 pow(1/2.2) 近似 gamma；游戏侧 spawn 默认日光渐变，Debug 面板经 `SkyParams` POD 镜像可调（含 enabled）；主 World 无 SkySettings 则跳过 pass 保留纯色 clear；天空为 view 级绘制不计入 `draw_calls`（保持 draw_calls == visible 语义）。统计面板（D7）：`FrameStats` 扩展 `pso_cache_size`/`tracked_memory_bytes`/`gpu_total_bytes`/`gpu_available_bytes`，`RenderFrameRunner::runFrame` 每帧自 device 填充；`buildRenderStatsPanel` 改收 `RenderStatsParams` POD（FPS/draw calls/visible/culled/total/PSO/显存，ImGuiLib 零引擎依赖），main.cpp.in 镜像接线。**计划外偏差（正路修复）**：D7 调研假设「PSOManager 统计现成」，实则 `Material::init` 直调 `device->createPipelineState`、PSOManager 是死代码——为让面板数字有意义，把 `GLRHIDevice::createPipelineState` 接入 `PSOManager::find`/`insert`（新增两方法 + 私有 `createPipelineStateUncached`，接口不变），相同 shader pair+状态共享一个 GL program，Sponza 28 材质+fallback+天空收敛为 **3 个 PSO**（单面/双面/天空）。**验收**：Debug 构建零编译/链接错误；EntelechyTests 223 全绿（220 基线 + 3 ExtractSkySystem）；lint 全仓 244 文件零违规。实跑（`logs/game_run_5c.log`）：零 ERROR/WARN、73 贴图全落地、60–61 fps 无回归、`draw_calls == visible`；截图（`logs/shot_5c_sky3.png`）确认 Render Stats 面板 FPS 61 / PSO Cache 3 / **GPU Tracked 6.22 GiB**（与 5b mip 链修复后约 6.2 GiB 预期吻合；本机无 NVX/ATI 扩展，GPU Total 显示 n/a 走预期 fallback 分支）、Debug 面板天空控件生效。已知环境问题同 5b（窗口启动后可能被最小化，需 ShowWindow 恢复，非 5c 引入）。**天空观感用户已目视确认（蓝色渐变，表现正常）**。新债务已记 TODO.md（天空盒未做——渐变 pass 为占位）；TODO.md 中 gamma 近似、ambient 常量化、光照 uniform 立即模式三条 5a 已记条目维持开放。
