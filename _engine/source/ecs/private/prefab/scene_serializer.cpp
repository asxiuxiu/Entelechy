...
bool SceneSerializer::deserialize(World &world, const String &jsonStr) const
{
    ...
    if (!cur.consume('{'))
        return false;
    String entitiesKey;
    if (!cur.parseString(entitiesKey))
        return false;
    if (!cur.consume(':'))
        return false;
    if (!cur.consume('['))
    ...
    while (true)
    {
        ...
        // Parse "id":N
        String key;
        if (!cur.parseString(key) || key != "id")
            return false;
        if (!cur.consume(':'))
            return false;
        u32 entityId = 0;
        if (!cur.parseUint32(entityId))
            return false;
        ...
        // Parse "components":{...}
        if (!cur.parseString(key) || key != "components")
            return false;
        if (!cur.consume(':'))
            return false;
        if (!cur.consume('{'))
