...
#define ASSERT_TRUE(expr) \
    do { \
        if (!(expr)) { \
            Entelechy::Test::TestRegistry::instance().reportFailure( \
                __FILE__, __LINE__, "ASSERT_TRUE failed: %s", #expr); \
            return; \
        } \
    } while(0)
...
#define ASSERT_FALSE(expr) \
    do { \
        if (expr) { \
            Entelechy::Test::TestRegistry::instance().reportFailure( \
                __FILE__, __LINE__, "ASSERT_FALSE failed: %s", #expr); \
            return; \
        } \
    } while(0)
...
#define ASSERT_EQ(a, b) \
    do { \
        if (!((a) == (b))) { \
            Entelechy::Test::TestRegistry::instance().reportFailure( \
                __FILE__, __LINE__, "ASSERT_EQ failed: %s != %s", #a, #b); \
            return; \
        } \
    } while(0)
...
#define ASSERT_NE(a, b) \
    do { \
        if ((a) == (b)) { \
            Entelechy::Test::TestRegistry::instance().reportFailure( \
                __FILE__, __LINE__, "ASSERT_NE failed: %s == %s", #a, #b); \
            return; \
        } \
    } while(0)
