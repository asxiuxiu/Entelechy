...
public:
    ...
    ~FixedRingQueue() { clear(); }

    FixedRingQueue(const FixedRingQueue&) = delete;
    FixedRingQueue& operator=(const FixedRingQueue&) = delete;

    FixedRingQueue(FixedRingQueue&& other) noexcept
        : m_head(0), m_count(other.m_count) {
    ...
    [[nodiscard]] T& front() { return *ptr(m_head); }
    [[nodiscard]] const T& front() const { return *ptr(m_head); }

    [[nodiscard]] T& back() { return *ptr((m_head + m_count - 1) % Capacity); }
    [[nodiscard]] const T& back() const { return *ptr((m_head + m_count - 1) % Capacity); }

    [[nodiscard]] T& operator[](usize i) { return *ptr((m_head + i) % Capacity); }
    [[nodiscard]] const T& operator[](usize i) const { return *ptr((m_head + i) % Capacity); }
    ...
    public:
        ...
        T& operator*() const { return (*m_queue)[m_idx]; }
        T* operator->() const { return &(*m_queue)[m_idx]; }
    ...
    public:
        ...
        const T& operator*() const { return (*m_queue)[m_idx]; }
        const T* operator->() const { return &(*m_queue)[m_idx]; }
    ...
    [[nodiscard]] Iterator end() { return Iterator(this, m_count); }
    [[nodiscard]] ConstIterator begin() const { return ConstIterator(this, 0); }
    [[nodiscard]] ConstIterator end() const { return ConstIterator(this, m_count); }
