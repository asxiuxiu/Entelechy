...
public:
    ...
    InlineArray(InlineArray &&other) noexcept
        : m_count(other.m_count), m_heap_data(other.m_heap_data), m_heap_capacity(other.m_heap_capacity)
    {
    ...
    [[nodiscard]] T *data()
    {
        return m_count <= N ? inlinePtr() : m_heap_data;
    }
    ...
    {
        return m_heap_capacity > 0 ? m_heap_capacity : N;
    }
...
private:
    ...
    T &inlineSlot(usize i)
    {
        return reinterpret_cast<T *>(m_inline)[i];
    }
    const T &inlineSlot(usize i) const
    {
    ...
    T &slot(usize i)
    {
        return (i < N && m_heap_data == nullptr) ? inlineSlot(i) : m_heap_data[i];
    }
    ...
    {
        if (needed <= N && m_heap_data == nullptr)
            return;
