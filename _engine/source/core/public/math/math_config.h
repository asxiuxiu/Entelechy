...
#include <cmath>
#define MATH_CHECK_FINITE_2(v) CHECK(std::isfinite((v).x) && std::isfinite((v).y))
#define MATH_CHECK_FINITE_3(v) CHECK(std::isfinite((v).x) && std::isfinite((v).y) && std::isfinite((v).z))
