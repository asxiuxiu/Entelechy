...
ImVec4 logLevelToColor(LogLevel level)
{
    ...
    {
    case LogLevel::Debug:
        return ImVec4(0.60f, 0.60f, 0.60f, 1.00f); // gray
    case LogLevel::Info:
        return ImVec4(0.90f, 0.90f, 0.90f, 1.00f); // near-white
    case LogLevel::Warning:
        return ImVec4(1.00f, 0.85f, 0.20f, 1.00f); // yellow
    case LogLevel::Error:
        return ImVec4(1.00f, 0.25f, 0.25f, 1.00f); // red
    }
                     ...
                     WindowSizeRequest &outRequest)
{
    ...
    if (ImGui::Combo("Preset", &selectedPreset, comboItems))
    {
        ...
        {
        case 0:
            outRequest.width = 1280;
            outRequest.height = 720;
            break;
        case 1:
            outRequest.width = 1600;
            outRequest.height = 900;
            break;
        case 2:
            outRequest.width = 1920;
            outRequest.height = 1080;
            break;
        case 3:
            outRequest.width = -1;
            outRequest.height = -1;
...
void buildLogPanel()
{
    ...
    while (clipper.Step())
    {
        ...
        for (const auto &entry : history)
        {
            ...
            ImGui::Text("[%s] [%s] [%s] %s: %s", timeStr, logLevelToShortString(entry.m_level),
                        entry.m_category_name ? entry.m_category_name : "?", entry.m_function ? entry.m_function : "?",
