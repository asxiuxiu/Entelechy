...
| `event_writer_reader.h` | `EventWriter<T>` / `EventReader<T>`：类型安全的事件总线包装 API |
...
- 改**Plugin 接口或生命周期** → 动 `plugin.h`、`app.h/.cpp`
- 改**事件总线 API** → 动 `event_writer_reader.h`

## 依赖关系

- 向上依赖：
  - [Core 模块](../core/AGENTS.md)（容器 `DynamicArray` / `HashMap`、字符串工具、数学类型、分配器）
- 被依赖：
...
- **Plugin 依赖排序**：`App::sortPlugins()` 按 `LoadingPhase` 粗分组，组内用 Kahn 拓扑排序 + 循环依赖检测。跨 `Phase` 的依赖声明会被忽略（因为阶段顺序已保证）。
- **事件总线分层设计**：当前用 ECS 事件实体实现（`KeyboardEvent` 组件 + `EventCleanupSystem`）。`EventWriter<T>` / `EventReader<T>` 是对此模式的类型安全包装。如高频事件成为瓶颈，可迁移到 `EventBuffer<T>` 环形队列。

## 技术债务

> 统一维护于 [TODO.md](../../../../TODO.md)。本模块相关条目包括：ECS/TypeRegistry、ECS/CommandBuffer、ECS/EventBus、ECS/PluginManifest、ECS/WorldComponentStorage、ECS/NoResourceConcept。
