...
**验证**：重新运行 `scripts/build/build.bat`，确认 `compile_commands.json` 中能找到 `glad/glad.h`。
...
> **注意**：当前 `main.cpp` 在 `launch/templates/` 下，是生成出来的。修改模板后需要重新运行 `scripts/build/build.bat` 生效。
...
4. **验证**：运行 `scripts/build/build.bat`，窗口应显示深灰色而非黑屏
5. **进入阶段 2**：接入 ImGui
